...
# --- AUXILIAR: PADRONIZAÇÃO DE DATAS ---
def ajustar_data_formatada(data_str):
    d_str = str(data_str).strip()
    if '-' in d_str:
        try:
            apenas_data = d_str.split(' ')[0]
            partes = apenas_data.split('-')
...
def upload_file(usuario_atual):
    ...
    try:
        for file in arquivos:
            ...
            df_padrao['data'] = df_padrao['data'].apply(ajustar_data_formatada)
        ...
        assinaturas_existentes = {
            (str(t.data).strip(), str(t.banco).strip(), str(t.descricao).strip(), round(float(t.valor), 2))
            for t in lista_completa_existente
        }
        ...
        for _, Server_Row in df_consolidado.iterrows():
            ...
            if assinatura_nova not in assinaturas_existentes:
                ...
                assinaturas_existentes.add(assinatura_nova)
                novas_adicionadas += 1
        ...
        print(f"--- {novas_adicionadas} novas transações inseridas sem duplicidade ---")
